...
public class SendingMapBehaviour extends SimpleBehaviour 
{
    ...
    public SendingMapBehaviour(Agent agent) 
    {
        ...
        System.out.println("(HarvesterCoordinator) Behaviour to send maps to Scouts");
    }

    @Override
    public void action() 
    { 
        System.out.println("(HarvesterCoordinator) Starting SendingMapBehaviour");
        HarvestCoordinator agent = (HarvestCoordinator) this.getAgent();
        if (first)
        {
            // Harvesters list initialization
        ...
        for (int i = 0; i < allHarvesters.size(); i++)
        {
            message.addReceiver(allHarvesters.get(i).getAID());
        }
        
        try {
            
            message.setContentObject(agent.getGame());
            agent.log("Sending new map to all Harvesters");
        } catch (Exception e) {
            e.printStackTrace();
        }
